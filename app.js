...
async function showApp(user){
  ...
  try{
    ...
    if(filterNameWrap){
      filterNameWrap.classList.toggle("hidden", !isAdmin)
    }
...
function getFilterValuesFromInputs(){
  return {
    ...
    name: isAdmin ? (qs("filterName")?.value || "").trim().toLowerCase() : "",
...
function buildReportData(){
  ...
  const data = {
    ...
    totalOreOrdinarie: rows.reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOreExtra: rows.reduce((acc, r) => acc + Number(r.ore_extra || 0), 0),
    totalOreFerie: rows.filter(r => r.stato === "Ferie").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOrePermessi: rows.filter(r => r.stato === "Permesso").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOreMalattia: rows.filter(r => r.stato === "Malattia").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOreLutto: rows.filter(r => r.stato === "Lutto").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOreMaternita: rows.filter(r => r.stato === "Maternità").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    totalOre104: rows.filter(r => r.stato === "104").reduce((acc, r) => acc + Number(r.ore || 0), 0),
    mese: appliedFilters.month || "",
    meseLabel: formatMonthLabel(appliedFilters.month || ""),
    periodoDal: appliedFilters.from || "-",
    periodoAl: appliedFilters.to || "-",
    dipendente: appliedFilters.employee || (isAdmin ? "Tutti" : getDisplayName(currentProfile, currentUser?.email || "-"))
  }
...
function buildReportText(includeZeroValues = false){
  ...
  const lines = [
    `Report presenze`,
    `Mese: ${report.meseLabel}`,
    `Periodo personalizzato: ${report.periodoDal} / ${report.periodoAl}`
  ]

  if(isAdmin){
    lines.push(`Dipendente: ${report.dipendente}`)
  }

  lines.push("")

  const totals = [
    ["Tot ore ordinarie", report.totalOreOrdinarie],
    ["Tot ore extra", report.totalOreExtra],
    ["Tot ore Ferie", report.totalOreFerie],
    ["Tot ore Permessi", report.totalOrePermessi],
    ["Tot ore Malattia", report.totalOreMalattia],
    ["Tot ore Lutto", report.totalOreLutto],
    ["Tot ore Maternità", report.totalOreMaternita],
    ["Tot ore 104", report.totalOre104]
  ]

  totals.forEach(([label, value]) => {
    if(includeZeroValues || Number(value) !== 0){
      lines.push(`${label}: ${formatNumberForReport(value)}`)
    }
  })

  if(lines[lines.length - 1] === ""){
    lines.push("Nessuna ora da mostrare")
  }
...
function buildReportHtml(){
  const report = buildReportData()
  const parts = [
    `<div style="font-weight:700;font-size:1.08em;margin-bottom:8px;">Report presenze:</div>`,
    `<div>Mese: ${report.meseLabel}</div>`,
    `<div>Periodo personalizzato: ${report.periodoDal} / ${report.periodoAl}</div>`
  ]

  if(isAdmin){
    parts.push(`<div>Dipendente: ${report.dipendente}</div>`)
  }
  ...
  if(!totals.length){
    parts.push(`<div>Nessuna ora da mostrare</div>`)
  }else{
    totals.forEach(([label, value]) => {
      parts.push(`<div>${label}: ${formatNumberForReport(value)}</div>`)
    })
  }
...
function generateReport(){
  if(qs("reportBox")) qs("reportBox").innerHTML = buildReportHtml()
  setAppStatus("Report generato")
}

async function copyReport(){
  try{
    const text = buildReportText(false)
...
function exportExcel(){
  try{
    if(typeof XLSX === "undefined"){
      setAppStatus("Libreria Excel non disponibile")
      return
    }
    ...
    const summaryRows = [
      ...
      ["Mese", report.meseLabel],
      ["Periodo personalizzato", `${report.periodoDal} / ${report.periodoAl}`],
      ["Dipendente", report.dipendente],
      [],
      ["Tot ore ordinarie", Number(report.totalOreOrdinarie || 0)],
      ["Tot ore extra", Number(report.totalOreExtra || 0)],
      ["Tot ore Ferie", Number(report.totalOreFerie || 0)],
      ["Tot ore Permessi", Number(report.totalOrePermessi || 0)],
      ["Tot ore Malattia", Number(report.totalOreMalattia || 0)],
      ["Tot ore Lutto", Number(report.totalOreLutto || 0)],
      ["Tot ore Maternità", Number(report.totalOreMaternita || 0)],
      ["Tot ore 104", Number(report.totalOre104 || 0)],
      [],
      ["Nome", "Data", "Stato", "Ore", "Ore extra", "Sede", "Note", "Email"]
    ]
    ...
    const monthPart = report.mese || "tutte"
    const fromPart = appliedFilters.from || "inizio"
    const toPart = appliedFilters.to || "fine"
    XLSX.writeFile(workbook, `presenze_${monthPart}_${fromPart}_${toPart}.xlsx`)

    setAppStatus("Excel esportato")
  }catch(err){
    console.error("EXPORT EXCEL ERROR", err)
    setAppStatus("Errore export Excel")
  }
}
...
function bindEvents(){
  ...
  if(qs("btnExportExcel")) qs("btnExportExcel").onclick = exportExcel
